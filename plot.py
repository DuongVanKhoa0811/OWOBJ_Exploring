...
def plot_prediction(image, scores, boxes, labels, ax=None, plot_prob=True):    #[1,3,967,800] [1,4,81], [1,4,4],[1,4]
    if ax is None:
        ax = plt.gca()
    plot_results(image[0].permute(1, 2, 0).detach().cpu().numpy(), scores.cpu(), boxes.cpu(), labels.cpu(), ax, plot_prob=plot_prob)

def plot_results(pil_img, scores, boxes, labels, ax, plot_prob=True, norm=True):
    from matplotlib import pyplot as plt
    image = plot_image(ax[0], pil_img, norm)
    colors = COLORS * 100
    if boxes is not None:
        ...
        for sc, cl, (xmin, ymin, xmax, ymax), c in zip(scores, labels, boxes, colors):
            ...
            ax[1].text(xmin, ymin, text, fontsize=5, bbox=dict(facecolor='yellow', alpha=0.5))
    ax.grid('off')
